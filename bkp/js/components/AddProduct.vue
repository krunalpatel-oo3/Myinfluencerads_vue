<template>
  <div>
    ...
    <main class="main">
      <!-- Project Add -->
      <section class="section-systemadd error-present">
        <div class="container">
              ...
              <div class="projectadd-row">
                ...
                <div class="projectadd-media">
                  <div class="projectadd-media-heading">
                    <div class="cus-title">Product thumbnail</div>
                  ...
                  <div v-show="imageFilesCount >= max_upload_files">
                    <div class="">You reached maximum of files</div>
                  </div>

                  <div
                    v-show="imageFilesCount < max_upload_files"
                  ...
                  >
                    <div class="add-text">
                      <div class="upload-icon">
                       <img :src="`${publicPath}vector.png`" alt="" />
                      </div>
                  ...
                  <div class="project-video-item" v-if="link_youtube != '' && link_youtube != null">
                    <iframe
                    ...
                    <div class="form-group">
                      ...
                      <div class="file-list">
                        <div
                          ...
                          :key="nextFilePdf.name"
                          v-if="nextFilePdf.type == 'application/pdf'"
                        >
                          <div class="file-w">
                            <img :src="`${publicPath}pdf.png`" alt="" />
                            <div class="btns">
                              <button class="btn btn-white-icon-rounded btn-sm" @click="delete_pdf(ind)">
                                <icon-base
                                  width="8.15"
                                  height="10"
                                  view-box="0 0 12 14.64"
...
export default {
  ...
  async created() {
    ...
    await this.getLoadAccesoryProductList();
  ...
  methods: {
    ...
    async publishProduct () {
      ...
      if (this.product_title === "") {
        ...
        this.$toast.error("Please enter minimum 2 and maximum 55 characters in product title");
        return;
      }
      ...
      if (!this.imageFilesCount) {
        this.$toast.error('Upload at least one product thumbnail')
        return
      }

      if(Math.sign(this.product_price.trim()) == -1){
        this.$toast.error("Please enter positive value in price");
      ...
      d.append("accessories", JSON.stringify(this.accessories))
      d.append("compatible_product", JSON.stringify(this.compatibles))
      d.append("pdf_files_count", this.pdfFilesCount)
      ...
      const res = await axios.post(window.api_path + "product/addproductlist", d, { headers })
      this.submitting = false

      if (res.status === 200) {
        if (res.data.code === 1) {
          this.$toast.success(res.data.message)
          await this.$router.push({ name: 'Product Page', params: { slug: res.data.data.slug }})
        } else {
    ...
    async getLoadAccesoryProductList() {
      const result = await this.callApi(
        "post",
        window.api_path + "getAccesoryProductsList",
        {mode: "only_name"}
      );
      if (result.status != 200) {
        this.$toast.error(result.data.message);
        this.accesoryProductList = [];
        this.isLoading = false;
        return;
      }
      this.accesoryProductList = result.data.products.map(p => {
        return { ...p, qty: 1 }
      });
    },
  ...
  computed: {
    ...
    filteredAccessories () {
      // return this.accesoryProductList.filter((i) => {
      return this.productList.filter((i) => {
        return (
          i.text
            .toLowerCase()
